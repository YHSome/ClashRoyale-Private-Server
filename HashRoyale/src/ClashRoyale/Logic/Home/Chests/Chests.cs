...
namespace ClashRoyale.Logic.Home.Chests
{
    public class Chests
    {
        ...
        public Chest BuyChest(int instanceId, Chest.ChestType type)
        {
            ...
            // "Unlock all cards" chest: any Giant chest in the shop costs 10 gold
            // and grants every card in the game (instead of the normal content).
            if (type == Chest.ChestType.Shop && mainchest.Name.StartsWith("Giant_"))
            {
                if (!Home.UseGold(10)) return null;

                var unlockChest = new Chest
                {
                    ChestId = instanceId,
                    Type = type
                };
