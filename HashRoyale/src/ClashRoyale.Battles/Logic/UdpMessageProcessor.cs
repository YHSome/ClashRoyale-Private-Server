...
namespace ClashRoyale.Battles.Logic
{
    public class UdpMessageProcessor
    {
        public static async void Process(IChannelHandlerContext ctx, DatagramPacket packet)
        {
            ...
            if (length > 17)
            {
                var dump = new byte[Math.Min(40, length)];
                content.GetBytes(content.ReaderIndex, dump);
                Logger.Log($"[CMD] from={packet.Sender} len={length} {BitConverter.ToString(dump)}", null,
                    ErrorLevel.Info);
            }
