        ...
        public void Add(SessionContext ctx, long sessionId)
        {
            lock (_syncObject)
            {
                ...
                else
                {
                    ...
                    var existing = session.Get(ctx.EndPoint);
                    if (existing != null)
                    {
                        // The client re-sends the registration packet (UDP
                        // retransmission). Keep the existing context so the
                        // battle state and command queues are not torn down.
                        ctx.Session = session;
                        return;
                    }
